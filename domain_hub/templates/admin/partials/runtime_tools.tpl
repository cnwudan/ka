...
$partnerPlanAdminEmailSetting = trim((string) ($module_settings['partner_plan_admin_email'] ?? ''));
$sponsorTitleSetting = trim((string) ($module_settings['sponsor_title'] ?? ''));
$sponsorDescriptionSetting = trim((string) ($module_settings['sponsor_description'] ?? ''));
$sponsorMethodsSetting = trim((string) ($module_settings['sponsor_methods'] ?? ''));
...
<div class="card mb-4" id="runtime-control">
  <div class="card-body">
    ...
    <form method="post" class="row g-3 align-items-center">
      ...
      <div class="col-12">
        <hr>
        <h6 class="text-muted mb-2"><i class="fas fa-handshake"></i> 合作伙伴计划 & 赞助展示</h6>
      </div>
      <div class="col-12 col-lg-6">
        <label class="form-label" for="partner_plan_admin_email">合作申请接收邮箱</label>
        <input type="text" class="form-control" id="partner_plan_admin_email" name="partner_plan_admin_email" value="<?php echo htmlspecialchars($partnerPlanAdminEmailSetting); ?>" placeholder="admin@example.com, partner@example.com">
        <small class="text-muted">支持多个邮箱，使用英文逗号、分号或换行分隔。</small>
      </div>
      <div class="col-12 col-lg-6">
        <label class="form-label" for="sponsor_title">赞助模块标题（双语）</label>
        <input type="text" class="form-control" id="sponsor_title" name="sponsor_title" value="<?php echo htmlspecialchars($sponsorTitleSetting); ?>" placeholder="赞助 DNSHE,Support DNSHE" maxlength="255">
        <small class="text-muted">格式：中文,English。仅填写一种语言时将共用。</small>
      </div>
      <div class="col-12">
        <label class="form-label" for="sponsor_description">赞助模块描述（双语）</label>
        <textarea class="form-control" id="sponsor_description" name="sponsor_description" rows="3" maxlength="3000" placeholder="中文描述,English description"><?php echo htmlspecialchars($sponsorDescriptionSetting); ?></textarea>
        <small class="text-muted">格式：中文,English。前台将按用户语言自动展示。</small>
      </div>
      <div class="col-12">
        <label class="form-label" for="sponsor_methods">赞助商名称 + 链接配置（支持双语）</label>
        <textarea class="form-control" id="sponsor_methods" name="sponsor_methods" rows="5" maxlength="6000" placeholder="支付宝,Alipay|https://example.com/alipay&#10;USDT|https://example.com/usdt"><?php echo htmlspecialchars($sponsorMethodsSetting); ?></textarea>
        <small class="text-muted">每行一项，格式：名称|链接。名称支持双语（中文,English）。链接仅支持 http/https。</small>
      </div>
